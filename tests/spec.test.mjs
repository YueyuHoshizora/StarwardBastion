...
test('敵人生命、護盾與移速依 round_half_up 計算', () => {
  ...
  assert.equal(scaledHp(195, 1, 2), 410); // 195 × 2.00 × 1.05 = 409.5 進位
  assert.equal(scaledHp(130, 2, 2), 307); // 130 × 2.25 × 1.05 = 307.125
  assert.equal(scaledHp(0, 5, 20), 0);
  assert.equal(scaledHp(49, 5, 1, 150), 221); // 關卡調校 150%：49 × 3.00 × 1.50 = 220.5 進位
  assert.equal(scaledHp(49, 5, 1, 100), scaledHp(49, 5, 1));
